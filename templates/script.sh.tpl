...
# ──────────────────────────────────────────────────────────────────────────────
# Métadonnées auto (dérivées des lignes d’en-tête ci-dessus)
SELF="${BASH_SOURCE[0]:-$0}"

# Récupère la valeur après '# VERSION=' / '# NAME=' ; valeurs de secours si absent
VERSION="${VERSION:-$(grep -m1 -E '^#\s*VERSION=' "$SELF" | sed -E 's/^#\s*VERSION=//')}"
NAME="${NAME:-$(grep -m1 -E '^#\s*NAME=' "$SELF"     | sed -E 's/^#\s*NAME=//')}"
: "${VERSION:=0.0.1}"
: "${NAME:=__NAME__}"

# Optionnel : charge DESCRIPTION / HOMEPAGE si utiles
...
Options:
  ...
  --no-medium           Example flag (disable “medium” set)
